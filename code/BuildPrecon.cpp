...
//	}
	...
	test.move_core(0);
	XERUS_LOG(info,"Approximation error = " <<std::setprecision(12) <<test.frob_norm());
	Fock.reinterpret_dimensions({4,4});
	Fock_inv.reinterpret_dimensions({4,4});
	Fock_inv2.reinterpret_dimensions({4,4});
	XERUS_LOG(info,Fock);
	XERUS_LOG(info,Fock_inv);
	XERUS_LOG(info,Fock_inv2);
...
TTOperator build_Fock_op_inv2(std::vector<value_t> coeffs, const size_t k, value_t shift){
	...
	for ( int j = -k_int; j <=k_int; ++j){
		...
		for (size_t i = 0; i < dim; ++i){
			...
			auto aa = xerus::Tensor({1,2,2,1});
			aa[{0,0,0,0}] = std::exp(j_v*h-std::exp(h*j_v)*shift/dim_v);
			aa[{0,1,1,0}] = std::exp(j_v*h-std::exp(h*j_v)*(coeffs[i]+shift/dim_v));
